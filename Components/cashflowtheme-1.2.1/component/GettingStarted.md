...
```csharp
CashflowTheme.Apply (UIProgressView.Appearance);
```
...
the following example, we apply `CashflowTheme` to `UIProgressView` only
when `UIProgressView` is a child of `UINavigationBar`:

```csharp
CashflowTheme.Apply (UIProgressView.AppearanceWhenContainedIn (typeof (UINavigationBar)));
```
